...
import { getDashboardStatsAPI, getAIFeedbackAPI } from '../api/dashboard'
import {
  ResponsiveContainer, BarChart, Bar, XAxis, Tooltip, CartesianGrid,
} from 'recharts'
import {
  Brain, Flame, CheckCircle2, Clock3, Sparkles, TrendingUp,
} from 'lucide-react'
...
const Dashboard = () => {
  ...
  const standupSubmitted = localStorage.getItem('standup_submitted_date') === today
      ...
      <main className="lg:ml-72 flex-1 p-4 lg:p-8 pt-24 lg:pt-8 relative">
        ...
        <div className="relative z-10 mb-10 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-5">
          ...
          <div className="flex items-center gap-3">
            {/* Standup Button - Employee only */}
            {user?.role === 'employee' && (
              <button
                onClick={() => setShowStandup(true)}
                disabled={standupSubmitted}
                className={`flex items-center gap-2 px-5 py-3 rounded-2xl font-semibold text-sm transition-all ${
                  standupSubmitted
                    ...
                    : 'bg-indigo-500 text-white hover:scale-[1.02] shadow-lg'
                }`}
              >
                {standupSubmitted ? '✅ Standup Submitted' : '📝 Submit Standup'}
              </button>
            )}
            <div className="flex items-center gap-3 bg-white/5 backdrop-blur-xl border border-white/10 px-5 py-3 rounded-2xl shadow-2xl">
              ...
              <span className="text-sm text-gray-300">AI Productivity Insights</span>
            </div>
          </div>
        </div>
        ...
        {loading ? (
          <div className="relative z-10 flex items-center justify-center h-[70vh]">
            <div className="flex flex-col items-center gap-4">
              ...
              <p className="text-indigo-300 animate-pulse">Loading productivity dashboard...</p>
            </div>
          </div>
        ) : (
          <>
            <div className="relative z-10 grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5 mb-8">
              <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-[28px] p-6 shadow-2xl hover:-translate-y-1 transition-all duration-300">
                ...
                <p className="text-gray-500 text-xs mt-2">Deep work today</p>
              </div>
              ...
              <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-[28px] p-6 shadow-2xl hover:-translate-y-1 transition-all duration-300">
                ...
                <h2 className="text-5xl font-bold text-white mt-2">{stats?.completed_tasks}</h2>
                <p className="text-gray-500 text-xs mt-2">Great consistency</p>
              </div>
              ...
              <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-[28px] p-6 shadow-2xl hover:-translate-y-1 transition-all duration-300">
                ...
                <h2 className="text-5xl font-bold text-white mt-2">{stats?.pending_tasks}</h2>
                <p className="text-gray-500 text-xs mt-2">Stay focused</p>
              </div>
              ...
              <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-[28px] p-6 shadow-2xl hover:-translate-y-1 transition-all duration-300">
                <div className="flex items-center justify-between mb-5">
                  ...
                  <span className="text-xs text-orange-400">Strong</span>
                </div>
                <p className="text-gray-400 text-sm">Productivity Streak</p>
                <h2 className="text-5xl font-bold text-white mt-2">
                  {stats?.streak}
                  <span className="text-lg text-gray-500 ml-1">days</span>
                </h2>
                <p className="text-gray-500 text-xs mt-2">Keep the momentum</p>
              </div>
            </div>
            ...
            <div className="relative z-10 grid grid-cols-1 xl:grid-cols-3 gap-6">
              <div className="xl:col-span-2 bg-white/5 backdrop-blur-xl border border-white/10 rounded-[36px] p-8 shadow-2xl">
                <div className="flex items-center gap-4 mb-8">
                  ...
                  <div>
                    <h2 className="text-3xl font-bold text-white">Weekly Focus Analytics</h2>
                    <p className="text-gray-400 mt-1">Your deep work activity over the last 7 days.</p>
                  </div>
                </div>
                <div className="h-87.5">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={chartData}>
                      ...
                      <Tooltip />
                      <Bar dataKey="minutes" radius={[12, 12, 0, 0]} fill="#8B5CF6" />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              </div>
              ...
              <div className="bg-linear-to-br from-indigo-500/20 to-violet-500/10 backdrop-blur-xl border border-white/10 rounded-[36px] p-8 shadow-2xl">
                <div className="flex items-center gap-4 mb-8">
                  ...
                  <div>
                    ...
                    <p className="text-gray-300 mt-1">Personalized productivity insights</p>
                  </div>
                </div>
        ...
        {showStandup && (
          <StandupModal
            ...
            onSubmit={() => {
              const today = new Date().toISOString().split('T')[0]
              localStorage.setItem('standup_submitted_date', today)
